# Myriad Cognitive Architecture - Complete Design & Architectural Blueprint (C#/.NET Edition)

**Version**: 5.0 C#
**Date**: 2025-01-01
**Status**: Neurogenesis + Enhanced Graph Intelligence + Hebbian Learning Operational

This document combines the foundational architectural blueprint with the advanced "Brain Approach" evolution strategy for the Myriad Cognitive Architecture, adapted for a C# and .NET technology stack.
...
**Phase 2 Neurogenesis: Biomimetic Intelligence**
...
The Myriad architecture is a multi-tiered, decentralized system of microservices, built on ASP.NET Core. The data flows through a series of specialized processors, activating concept agents as needed.
...
The heart of the system. An agent is an independently deployable ASP.NET Core microservice embodying a single concept. They are heterogeneous.
...
**🆕 IMPLEMENTED**: The GraphDB Manager AI, an ASP.NET Core service, serves as the interface between the cognitive architecture and the Neo4j knowledge graph. It replaces the simple agent registry with sophisticated relationship-based discovery using the `Neo4jClient` library.
...
**🚀 MICROSERVICE EXTRACTED**: The Orchestrator has been extracted from the Integration Tester into a standalone, scalable ASP.NET Core microservice with a full REST API (Port 5000).

-   **Standalone Service:** Independent ASP.NET Core application with dedicated health checks (`/health`), metrics, and monitoring.
...
2.  **Orchestration & Routing:**
    -   GraphDB lookup finds agents: `Apple_Company_AI`, `Marketing_Campaign_AI`, `Brand_Psychology_AI`.
    -   Router dispatches parallel requests with context using `HttpClientFactory`.
...
-   **Current State (v1.0):** The Orchestrator uses synchronous REST calls (e.g., `HttpClient.PostAsync().Result`). It calls one agent and waits for the response before calling the next, or handles a small number of parallel calls. This is simple but slow.

-   **Next Step (Asynchronous I/O):** Rearchitect the Orchestrator to use C#'s `async`/`await` pattern with `Task.WhenAll` to fire off requests to all required agents simultaneously and process their responses as they arrive. This dramatically improves performance and mimics the brain's parallel processing.

-   **Advanced State (Event-Driven Architecture):** Replace direct calls with a message broker (e.g., RabbitMQ, Azure Service Bus). The Orchestrator publishes a "query event" to a public topic. Agents subscribe to topics they are interested in and publish their findings to a "synthesis" topic. This fully decouples all components, removing the Orchestrator as a bottleneck and more closely modeling the synaptic release of neurotransmitters.
...
-   **Advanced Optimization:** Implement distributed caching (e.g., Redis) for graph lookups and auto-scaling with Kubernetes for handling large agent networks.
...
-   **Step 1.1: Implement the Graph Database Core**
    -   **Task 1.1.1:** Integrate a graph database (e.g., Neo4j) into the system via `docker-compose.yml`.
    ...
    -   **Task 1.1.3:** Create a new `GraphDB_Manager_AI` service in C#. This Genesis Agent will be the sole interface for all read/write operations to the graph, ensuring consistency and centralizing logic.

-   **Step 1.2: Rearchitect the Orchestrator as a "Graph Traverser"**
    -   **Task 1.2.1:** Modify the Orchestrator service. Remove direct `HttpClient` calls to agent microservices for knowledge retrieval.
...
-   **Step 1.3: Migrate Existing Agents to Graph Nodes**
    -   **Task 1.3.1:** Write a migration script in C#. This script will read the capabilities of the existing `Lightbulb_Definition_AI` and `Lightbulb_Function_AI`.
